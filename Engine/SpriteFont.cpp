...
namespace Engine {
    ...
    void SpriteFont::draw(SpriteBatch& batch, const char* s, glm::vec2 position, glm::vec2 scaling,
        float depth, Engine::Vertex::ColorRGBA8 tint, Justification just /* = Justification::LEFT */) {
        ...
        for (int si = 0; s[si] != 0; si++) {
            ...
            if (s[si] == '\n') {
                ...
                std::vector<Engine::Vertex::ColorRGBA8> tintsPerEveryVertex= tint.getVectorWithSameColor(tint);
                batch.draw(_texID, depth, destRect, _glyphs[gi].uvRect, tintsPerEveryVertex);
