...
void BallController::updateBalls(std::vector <Ball>& balls, Grid* grid, float deltaTime, int maxX, int maxY) {
    ...
    for (size_t i = 0; i < balls.size(); i++) {
        ...
        // Check to see if the ball moved TO OTHER CELL 
        Cell* newCell = grid->getCell(ball.position);

        if (newCell != ball.ownerCell) {
            //NEED TO SHIFT THE BALL, REMOVE IT FROM THAT VECTOR AND PUT IT TO THE VECTOR TO THE CELL THAT IS GOING TO
            grid->removeBallFromCell(&balls[i]);

            //add to new cell
            //add ball to new and determine which cell belongs to
...
void BallController::updateCollision(Grid* grid) {
    // iterate over every cell and update them with L shape
    for (size_t i = 0; i < grid->m_cells.size(); i++) { //m_cells frien class BallCOntroler imamo enkapsulaciju
        //i je 1d array

        int x = i % grid->m_numXCells; //how far are we in that row (col)
        int y = i / grid->m_numXCells; // how many rows we have
        ...
        for (size_t j = 0; j < cell.balls.size(); j++) {
            ...
            // check collisiosn ball and vector of balls starting at specific index
            ...
            if (x > 0) {
                ...
                if (y > 0) {
                    /// Top left // 0 jer je to drugi cell encemo se provjerit sa sobom
...
void BallController::checkCollision(Ball& b1, Ball& b2) {
    ...
    if (collisionDepth > 0) {
        ...
        // transfer color to the sloer ball
