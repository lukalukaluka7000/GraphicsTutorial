...
MainGame::~MainGame() {
    // Empty
    //kad se god destruktor pozove unique_ptr se automatski mice
}
...
void MainGame::init() {
    ...
    m_window.initWindow("Ball Game", m_screenWidth, m_screenHeight, Engine::FULLSCREEN);
...
struct BallSpawn {
    BallSpawn(const Engine::Vertex::ColorRGBA8& colr,
...
#define ADD_BALL(p, ...) \
    ...
    /*const Engine::Vertex::ColorRGBA8& colr,
        float rad, float m, float minSpeed,
        float maxSpeed, float prob)*/

    // Adds the balls using a macro
    ADD_BALL(1.0f, Engine::Vertex::ColorRGBA8(255, 255, 255, 255),
             10.0f, 1.0f, 0.1f, 7.0f, totalProbability);
    ADD_BALL(1.0f, Engine::Vertex::ColorRGBA8(1, 254, 145, 255),
             10.0f, 2.0f, 0.1f, 3.0f, totalProbability);
    ADD_BALL(1.0f, Engine::Vertex::ColorRGBA8(177, 0, 254, 255),
             9.0f, 4.0f, 0.0f, 0.0f, totalProbability)
    ADD_BALL(1.0f, Engine::Vertex::ColorRGBA8(254, 0, 0, 255),
             9.0f, 4.0f, 0.0f, 0.0f, totalProbability);
    ADD_BALL(1.0f, Engine::Vertex::ColorRGBA8(0, 255, 255, 255),
             5.0f, 4.0f, 0.0f, 0.0f, totalProbability);
    ADD_BALL(1.0f, Engine::Vertex::ColorRGBA8(255, 255, 0, 255),
             5.0f, 4.0f, 0.0f, 0.0f, totalProbability);
    // Make a bunch of random ball types
    for (int i = 0; i < 10000; i++) {
        ADD_BALL(1.0f, Engine::Vertex::ColorRGBA8(r2(randomEngine), r2(randomEngine), r2(randomEngine), 255),
    ...
    // extra allocations.
    //jako bitno da bude dovoljno veliko jer nesmis npr u for petlji minjat size od vektora
    //Nego je bitno da unaprid rezerviras dovoljno
    ...
    for (int i = 0; i < NUM_BALLS; i++) {
        ...
        // Add the ball do the grid. IF YOU EVER CALL EMPLACE 
        // BACK AFTER INIT BALLS, m_grid will have DANGLING POINTERS!
        //zadnji element jer smo ga u prethodnim redovima dodali
...
void MainGame::drawHud() {
    const Engine::Vertex::ColorRGBA8 fontColor(255, 0, 0, 255);
